...
# -------------------------------------------
#-----주소---
html_page = read_html(request_url[2])

#---제목---
nodes_ttl <- html_nodes(html_page,
                        "h1")
title<- html_text(nodes_ttl)
#--user rate---
...
for(i in 1:length(request_url)){
  tempdf<-rbind(tempdf, moviecrawl(i))
 movieresult_df <- cbind(movieresult_df, tempdf)
}

movieresult_df <-as.data.frame(tempdf)
...
names(movieresult_df)<-c("title", "userRating", "Genre")
